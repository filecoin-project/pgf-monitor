...
# These exercise merge and date-normalization semantics, not registry validity, and the fixture
# triple below is one of the retired ones (OSO-5005). `allow_undeclared=True` is the documented
# escape hatch for exactly this: the subject under test is the table, not the contract.
...
def test_a_rerun_on_the_same_day_updates_rather_than_appends(tmp_path):
    ...
    append_observations([_obs(observed_value=0.9)], path, allow_undeclared=True)
    rows = append_observations([_obs(observed_value=0.999)], path, allow_undeclared=True)
    assert len(rows) == 1
...
def test_an_instant_and_its_date_are_the_same_row(tmp_path):
    ...
    rows = append_observations([_obs(observed_at="2026-07-19")], path, allow_undeclared=True)
    assert len(rows) == 1
...
def test_method_separates_provenance(tmp_path):
    path = tmp_path / "observations.csv"
    append_observations([_obs(method="nightly")], path, allow_undeclared=True)
    rows = append_observations([_obs(method="replay:api.github.com")], path, allow_undeclared=True)
...
def test_history_is_preserved_across_appends(tmp_path):
    path = tmp_path / "observations.csv"
    append_observations([_obs(observed_at="2026-08-12")], path, allow_undeclared=True)
    append_observations([_obs(observed_at="2026-08-13")], path, allow_undeclared=True)
    rows = append_observations([_obs(observed_at="2026-08-14")], path, allow_undeclared=True)
...
def test_a_null_reading_round_trips_as_empty(tmp_path):
    ...
    rows = append_observations(
        [_obs(observed_value=None, note="fetch_error: boom")], path, allow_undeclared=True
    )
