...
def test_draft_is_promotable_shaped(path):
    manifest, x_draft = split_draft(path)  # raises ManifestError on schema breakage
    declared = set(x_draft.get("allowlist_additions") or [])
    problems = promotion_problems(manifest, load_kernel(), allowlist=set())
    hard = [p for p in problems if "allowlist" not in p]
    assert hard == []
    # every non-allowlisted host must be declared in x_draft.allowlist_additions
    # (or already on the committed allowlist — checked by validate_draft at promotion)
    for fn in manifest.functions:
        if fn.source.kind == "http-json":
            cfg = build_ingestion_config(
                fn, window_for(fn.sla.cadence, datetime(2026, 7, 15, tzinfo=timezone.utc)), manifest.team
            )
